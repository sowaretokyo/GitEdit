...
- origin 以外のリモートを追跡しているブランチで、pull / push が常に origin に対して実行され失敗・誤送信し得た問題を修正（追跡先の remote / branch に追従。upstream ブランチ名がローカルと異なる場合の push 失敗も解消）
- リモートブランチはあるが未 push のローカルコミットが残った状態で、古い内容のまま PR を作成できてしまう問題を修正（先に push するよう促してブロック）
- 同一リポジトリでファイルシステム監視が二重に起動し、変更検知時の再読み込みが重複していた問題を修正
...
[Unreleased]: https://github.com/sowaretokyo/GitEdit/compare/v0.2.0...HEAD
[0.2.0]: https://github.com/sowaretokyo/GitEdit/compare/v0.1.5...v0.2.0
